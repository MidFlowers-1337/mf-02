...
router.get('/api/seats', authMiddleware, async (ctx) => {
  const seats = db.prepare('SELECT * FROM seats ORDER BY seat_number').all();
  ctx.body = seats;
});

router.get('/api/seats/:id', authMiddleware, async (ctx) => {
  const seat = db.prepare('SELECT * FROM seats WHERE id = ?').get(ctx.params.id);
  if (!seat) {
    ctx.status = 404;
...
router.put('/api/seats/:id', authMiddleware, adminMiddleware, async (ctx) => {
  ...
  const existing = db.prepare('SELECT id FROM seats WHERE seat_number = ? AND id != ?').get(seat_number, ctx.params.id);
  if (existing) {
    ctx.status = 400;
  ...
  ctx.body = db.prepare('SELECT * FROM seats WHERE id = ?').get(ctx.params.id);
});

router.post('/api/seats', authMiddleware, adminMiddleware, async (ctx) => {
  const { seat_number, zone, has_monitor, by_window, status } = ctx.request.body;
  const existing = db.prepare('SELECT id FROM seats WHERE seat_number = ?').get(seat_number);
  if (existing) {
    ctx.status = 400;
    ctx.body = { error: '工位编号已存在' };
    return;
  ...
  ctx.body = db.prepare('SELECT * FROM seats WHERE id = ?').get(info.lastInsertRowid);
});
